...
const getMachinesByCategory = async (req, res) => {
    const { page, limit, filterParams, filter,mainfilter,catId } = req.query;
    const { companyCode } = req.userInfo;
    const resdata = await getPaginatedData(companyCode.toUpperCase(), 'machines', page, limit, filterParams, filter, "catId", catId||mainfilter)
    return resdata ? res.status(200).send(resdata) : res.status(404);
}
// const get_customer_by_id = async (req, res) => {
//     console.log(req.query,"---------------");
//     console.log(req.query,"----------------------")
//     const { cid ,page, limit, filterParams } = req.query; // Get customer ID from query parameters
//     const { companyCode } = req.userInfo; // Extract company code from user info

//     // Check if customer ID is provided
//     if (!cid) {
//         return res.status(400).json({ error: 'Customer ID is required' }); // Bad request if no CID
//     }

//     try {
//         // Fetch customer data using the CID and company code
        
//         const customerData = await fetchCustomerDataById(cid, companyCode);

//         // Check if customer data was found
//         if (!customerData) {
//             return res.status(404).json({ error: 'Customer not found' }); // Not found if no customer data
//         }

//         // Respond with the customer data
//         return res.status(200).json(customerData); // Send the customer data back
//     } catch (error) {
...
// Example function to simulate fetching customer data from a database
const fetchCustomerDataById = async (cid, companyCode) => {
    // Example database query (replace with your actual DB query logic)
    const customer = await db.query('SELECT * FROM customers WHERE id = ? AND company_code = ?', [cid, companyCode]);
    return customer[0]; // Return the first matching customer
};
...
const addCustomer = async (req, res) => {
    ...
    try {
        const { cid, cname, c_email, city, phone, contactperson, address } = req.body;
        console.log(req.body.cid)
        if (req.body.cid) {
            // Update existing Customer
            // console.log(req.body.cid)
            console.log(companyCode, "------------------------********")
            const existingCustomer = await collection.findOne({ cid: Number(req.body.cid) });
            if (!existingCustomer) {
                return res.status(404).json({ message: 'Customer not found' });
            }


            // Prepare the customer update data, excluding _id
            ...
            // Update the Customer in the database, excluding _id
            const result = await collection.findOneAndUpdate(
                { cid: Number(req.body.cid) },
                { $set: updatedCustomerData },
                { returnDocument: 'after' }
            );
            return res.status(200).json({ message: 'Customer updated successfully', customer: result.value });
        } else {
            // Add new Customer

            const latestCid = await collection.find().sort({ cid: -1 }).limit(1).toArray();
            const newCid = (latestCid[0]?.cid || 0) + 1; // Increment the latest Cid

            // Prepare the Customer data, ensuring prodId is a number
            const newCustomerData = {
                ...req.body,
                cid: newCid, // Set new prodId


            };

            // Insert the machine data into the collection
            const result = await collection.insertOne(newCustomerData);

            return res.status(200).json({ message: 'Customer added successfully', customer: result });
        }
    } catch (error) {
...
const getStatusForSection = async (req, res) => {
    const { section } = req.query;
    // Log the sections for debugging
    console.log("Sections received:", section);

    try {
        // Connect to the database
        const db = client.db('machine_rental');
        const collection = db.collection('machineStatus');
        
        // If section is a comma-separated string, split it into an array
        const sectionsArray = section.split(',').map(sec => sec.trim()); // Trim whitespace

        console.log("Querying status for sections:", sectionsArray);
        
        // Fetch machine details by sections using the $in operator
        const statusData = await collection.find({ section: { $in: sectionsArray } }).toArray();
        ...
        // Return machine data without _id
        const sanitizedData = statusData.map(({ _id, ...rest }) => rest); // Exclude _id from the returned data
...
const getCustomerById = async (req, res) => {
    console.log(req.query,"+++++++++++++++++++++++++")
    const { companyCode } = req.userInfo;
    const { cid} = req.query; // Ensure you're getting prodId from the query string
    ...
    try {
        ...
        // Fetch machine details by cid
        const customer = await collection.findOne({ cid: Number(cid) }); // Make sure to match the field name in the DB
        ...
        // Return customer data without _id
...
const changeStatus = async (req, res) => {
    ...
    try {
        ...
        if (currentDoc && currentDoc.status !== status) {
            const result = await Collection.updateOne(
                { [req.body.uniqueRow]: req.body.id },
